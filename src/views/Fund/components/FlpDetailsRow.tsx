...
export const FlpDetailsRow: React.FC<FlpDetailsRowProps> = ({ row, isExpanded, colSpan, isTablet }) => {
	...
	const detailsRenderContent = (
		<>
			...
			<S.DetailsContent isTablet={isTablet}>
				{flpInfoLoading ? (
					<>
						<div>
							<S.SocialLinks>
						...
						<S.DetailsSectionsGrid>
							<div>
								...
								<S.DetailSectionContent isTablet={isTablet}>
									<div>
	...
	if (isTablet) {
		return <div>{detailsRenderContent}</div>;
	}

	return (
		<S.DetailsRow>
			<S.DetailsCell colSpan={colSpan}>{detailsRenderContent}</S.DetailsCell>
		</S.DetailsRow>
	);
};
