...
class Hexocet {
	...
	isOnScreen(x: number, y: number): boolean {
		...
		return (
			x >= 0 && x <= (this.canvas ? this.canvas.width : 0) && y >= 0 && y <= (this.canvas ? this.canvas.height : 0)
		);
	}
	...
	draw(): void {
		if (this.ctx && this.canvas) {
			...
			for (var key in this.seeds) {
				...
				const randomColor = Math.floor(Math.random() * this.secondaryColorsSL.length);

				// HSLA
				var hsla = {
					h: seed.hue,
					s: this.shouldUseSecondaryColor ? this.secondaryColorsSL[randomColor].s : '100%',
					l: this.shouldUseSecondaryColor ? this.secondaryColorsSL[randomColor].l : '100%',
